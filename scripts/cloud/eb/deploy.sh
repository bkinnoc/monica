...
if [ -z "${EB_DEPLOY_TIMEOUT}" ]; then
    EB_DEPLOY_TIMEOUT=30
fi
...
eb deploy "${ENVIRONMENT_NAME}" --timeout "${EB_DEPLOY_TIMEOUT}"
...
eb deploy "${WORKER_ENVIRONMENT_NAME}" --timeout "${EB_DEPLOY_TIMEOUT}"
