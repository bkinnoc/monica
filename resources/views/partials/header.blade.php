        ...
        <div class="mw9 center dt w-100">
            <div class="w-50-l w-100 dtc-l v-mid">
                <div class="tc dib-l">
                    <a href="{{ route('dashboard.index') }}"
                        class="header-logo {{ htmldir() == 'rtl' ? 'ml2' : 'mr2' }}">
                ...
                <div class="dib w-60-l w-100 header-search">
                    <contact-search placeholder="{{ trans('people.people_search') }}"
                        form-name-order="{{ App\Helpers\FormHelper::getNameOrderForForms(auth()->user()) }}">
                    </contact-search>
                </div>
            </div>
            <div class="w-50-l w-100 dtc-l v-mid tl tr-l">
                <div class="header-nav">
                    <a href="{{ route('dashboard.index') }}"
                        class="header-nav-item-link dib">{{ trans('app.main_nav_dashboard') }}</a>
                    <a href="{{ route('people.index') }}"
                        class="header-nav-item-link dib">{{ trans('app.main_nav_family') }}</a>
                    <a href="{{ route('journal.index') }}"
                        class="header-nav-item-link dib">{{ trans('app.main_nav_journal') }}</a>
                    @hasanyrole('Admin|Super Admin|Web Admin|Api Admin')
                        <a href="/nova" class="header-nav-item-link dib">{{ trans('app.main_nav_nova_admin') }}</a>
                    @endrole

                    {{-- NON MOBILE --}}
                    <a href="{{ route('settings.index') }}" class="header-nav-item-link dib-l dn">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                            aria-hidden="true" width="14" height="16"
